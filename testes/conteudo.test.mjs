...
const { esquemaSite, esquemaTextos, linkWhatsapp, ErroDeConteudo } = await import(
...
const recusaTextos = (bruto, idioma = 'pt-br') => recusaComo((v) => esquemaTextos(v, idioma), bruto)
...
test('urlBase aceita o caminho do GitHub Pages de projeto', () => {
  ...
  assert.equal(esquemaSite(site, 'site').meta.urlBase, 'https://navesz.github.io/rebar-site')
})
...
const lerTextos = (idioma) =>
  JSON.parse(readFileSync(join(RAIZ, 'conteudo', 'textos', `${idioma}.json`), 'utf8'))
